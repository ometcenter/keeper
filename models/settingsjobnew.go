...
func (S SettingsJobsAllV2) SaveSettingsToPg(DB *sql.DB) error {
	...
	var counter int
	DB.QueryRow("SELECT count(*) FROM settings_jobs777 WHERE job_id = $1", argsquery...).Scan(&counter)
	...
	if counter != 0 {
		...
		_, err := DB.Exec(`UPDATE settings_jobs777 SET job_id = $1, code_external = $2, 
		...
		_, err := DB.Exec(`INSERT INTO settings_jobs777 (job_id, code_external, name_external, table_name, json_byte)
